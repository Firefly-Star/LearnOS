...
### 4.1 概述
`进程管理` 是操作系统内核中最为核心的部分之一，它负责进程的创建、调度、切换、同步、通信以及销毁等操作，是操作系统实现多任务处理的基础。在 SupremeOS 中，我们实现了进程优先级调度以及vfork，为操作系统提供了强大的进程管理能力。
...
xv6 原先的进程调度为FIFO，即按照进程创建的顺序进行调度，而 `SupremeOS` 则实现了基础的优先级调度，每个进程都有一个优先级，优先级越高的进程优先被调度。
...
<div align = center>
<img width=750 src="images/fork和vfork.png">
<p style="font-size: 12px">图4.1 fork和vfork</p>
</div>
...
测试结果如图：
![shmtest1](images/shmtest1.png)，符合预期。
...
结果如下：
![semtest1](images/semtest1.png)

如果没有这个互斥锁，两个进程将竞争使用串口输出，会出现以下结果：
![semtest2](images/semtest2.png)
...
结果为：
![vforktest1](images/vforktest1.png)

如果换为fork，则结果为：
![vforktest2](image2/vforktest2.png)
