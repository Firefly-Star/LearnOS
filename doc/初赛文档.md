...
- 性能优化
通过使用懒分配和写时复制技术，减少了内存分配的开销，提高了内存分配的效率。同时，通过优化伙伴系统和`slab`分配器的算法，提高了内存分配的命中率，降低了内存碎片化程度，提高了内存的利用率。
...
用户虚拟地址空间的布局如下图所示，其中包括各个段的分配与映射。
...
`xv6-riscv`本来的进程调度为轮询策略，没有实现复杂的调度算法，而 `SupremeOS` 则计划实现基于优先级的多级反馈队列调度算法。将进程分为多个优先级队列，每个队列中的进程按照优先级进行调度，优先级越高的进程优先被调度。还计划实现进程的抢占式调度，即当一个高优先级的进程就绪时，会立即抢占正在运行的低优先级进程，从而保证高优先级进程的及时响应。
...
`xv6` 原先的进程调度为轮询，依次执行每个进程，而 `SupremeOS` 则实现了基础的优先级调度，每个进程都有一个优先级，优先级越高的进程优先被调度。
...
- [x] 实现基于优先级的多级反馈队列调度算法，允许抢占式调度，提高系统的响应速度和吞吐量
